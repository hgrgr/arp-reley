...
int main(int argc, char* argv[]) {
	...
	for(int i=0; i< size;i++)
	{
		...
		find_mac(argv[i*2+2],sender_mac,i);// find sender mac
		...
		for(int k=0;k<4;k++)
		{
			printf("%d ",pArr[i].ip[k]);
		}

		printf("\ntable sender_mac = ");
		for(int j=0;j<6;j++)
		{
			printf("%x ",pArr[i].mac[j]);
		}	
		printf("\n");

		find_mac(argv[i*2+3],target_mac,i+1);// find target mac
		...
		for(int k=0;k<4;k++)
		{
			printf("%d ",pArr[i+1].ip[k]);
		}
		printf("\n table sender_mac = ");
		for(int j=0;j<6;j++)
		{
			printf("%x ",pArr[i+1].mac[j]);
		}	
		printf("\n");
...
int find_relay_packet(const u_char* packet){
	for(int i=0; i< size; i++)
	{
		...
		{
			if(!(memcmp(,&packet[30],4)))//target check
			{
			
				printf("\n find sender mac1 = ");
				for(int k=0;k<4;k++)
				{
					printf("%x ",pArr[i*2+1].mac[k]);
				}
				printf("\n");
...
void send_relay(){
	while (true) {
        ...
        if(int size = find_relay_packet(packet))// find relay packet;
        {
			printf("find");	
            //break;
